...
<script>
  ...
  onMount(async () => {
    userPosts = await makeApiRequest(`/posts/user/${user.id}`, null, {
      method: 'GET',
    }).catch((err) => globalErrorHandler(err));
    if (!userPosts) return;
...
<div class="sponsor">
  ...
  <p>
    Sponsored posts cost $COST_SPONSORED_POST each and will be rotated randomly along with other
    sponsored posts throughout the category. It does not expire.
  </p>

  <p>
    We currently get around 12k pageviews a month. 20% of profits get distributed back to active
    members based on karma.
  </p>
  ...
  <form>
    ...
    <ul>
      {#each selectedPosts as post}
        <li>
          {post.title} <a href="javascript:void(0)" on:click={() => removeFromSelected(post)}>X</a>
        </li>
      {/each}
    </ul>
  ...
  {#if showBTC && qr}
    <div>
      ...
      <div class="payment">
        ...
        <h4 class="status">
          Status: {payStatus}
          {#if isLoadingStatus}<Spinner /> Awaiting payment...{/if}
        </h4>
        {#if payStatus === 'paid'}
          <p>
            Once an invoice is marked as "paid" you are done; however we wait for 1 confirmation to
            complete your purchase, which can take 10-30 minutes.
          </p>
          <p>
            You may check your payment status for confirmation on the <a href="/settings/invoices"
              >invoices</a
            > page.
          </p>
        {/if}
