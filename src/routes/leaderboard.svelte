...
async function getExchangeRate(dollar){
  const res = await fetch('https://blockchain.info/tobtc?currency=USD&value='+dollar)
    .then(res => {
      if (!res.ok) {
        console.error(res);
      }
...
async function getKarmaTransactions() {
  ...
  const btcAddresses = leaders.filter(user => user.bitcoinAddress).map(user => {
      return {
        karma: user.karma,
  ...
  btcAddresses.filter(user => user.payout > 0.00546).map(user => {
    debugger;
...
<form on:submit|preventDefault={getKarmaTransactions} class:active={show.karma}>
  ...
  <span class="mbtc">mBTC: {mbtc}</span>
  <button>Go</button>
  <textarea id="addressesKarma">{addressesKarma}</textarea>
  <a href="#" on:click|preventDefault={() => copyToClipboard(addressesKarma)}>copy</a>
  <div class="total">Total: {mbtcKarmaTotal.toFixed(4)} mBTC (minus dust). {karmaMinusDust} karma (minus dust) out of {totalKarma} total karma.</div>
</form>
