<script>
  ...
  $: {
    ...
    getVideo(post);
    getUserVote(post);
    if (!post.category) post.category = { name: '[deleted]' };
  }
      ...
      <div class="post-info">
        ...
        <div class="lower">
          <div class="lower-left">
            <a
              class="category-link"
              href={`/a/${post.category.name}`}
              on:click={(e) => (post.category.name == '[deleted]' ? e.preventDefault() : null)}
          ...
          <div class="lower-right">
            <a
              rel="prefetch"
              href={post.category ? `/a/${post.category.name}/${post.id}` : ''}
              class="comment-count"
            >
              <svg
                width="16"
